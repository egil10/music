...
async function main() {
  ...
  console.log("Streaming and aggregating…");
  let processedCount = 0;

  await new Promise((resolve, reject) => {
    const src = fs.createReadStream(RAW);
    pipeline(
      src,
      parser(),
      async function (err, dataStream) {
        ...
        dataStream.on("data", ({ value }) => {
          // Handle the nested structure where streaming_history is the key
          if (value && value.streaming_history && Array.isArray(value.streaming_history)) {
            console.log(`Processing ${value.streaming_history.length} streaming history items...`);
            value.streaming_history.forEach(item => {
              processedCount++;
